...
if st.session_state.hits:
    ...
    with left:
        ...
        for i in st.session_state.hit_keys:
            ...
            phrase = html.escape(h.text if len(h.text) <= 160 else h.text[:157] + "…")
            label = CATEGORY_LABELS.get(h.category, h.category)
            page = (h.page + 1) if h.page is not None else 1

            row = st.columns([0.10, 0.90])
            with row[0]:
                checked = st.checkbox("", key=f"hit_{i}", value=(i in st.session_state.selected_hit_ids))
                if checked:
                    new_selected.add(i)
            with row[1]:
                st.markdown(
                    ...
                    <div class="hit-row">
                      <div class="hit-card" style="--cat:{color}">
                        ...
                        <div class="meta">
                          <span class="badge" style="--cat:{color}">{html.escape(label)}</span>
